import { TourGuide } from "@/lib/interfaces/people/staff";

export const londonunitedKingdomnorthernEuropeGuides: TourGuide[] = [
  {
    id: "guide-london-1",
    name: "David Martinez",
    city: "london",
    country: "United Kingdom",
    state: "",
    region: "Northern Europe",
    isPopular: false,
    bio: "David is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Hidden Gems, Film Locations, David provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Middle East"],
    tourTypes: ["Private Tour", "Historical Tour", "Underground Tour"],
    maxGroupSize: 14,
    licenseNumber: "LIC-6332-LON",
    phoneNumber: "+1-707-241-8949",
    contactEmail: "david.martinez@paragontrails.com",
    available: [
      {
        day: "Saturday",
        availableHours: [
          {
            from: "09:00",
            to: "15:00",
          },
          {
            from: "14:00",
            to: "16:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "09:00",
            to: "11:00",
          },
        ],
      },
      {
        day: "Wednesday",
        availableHours: [
          {
            from: "12:00",
            to: "18:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "David is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Hidden Gems, Film Locations, David provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.3,
    reviewsCount: 35,
    quote:
      "The best way to discover london is with someone who lives and breathes its culture.",
    profileImage: "https://randomuser.me/api/portraits/men/22.jpg",
    languages: ["Finnish", "Indonesian", "Malay"],
    experienceYears: 16,
    certifications: ["Language Proficiency Certification"],
    specialties: ["Hidden Gems", "Film Locations"],
    specialTraining: [],
  },
  {
    id: "guide-london-2",
    name: "Elijah Lewis",
    city: "london",
    country: "United Kingdom",
    state: "",
    region: "Northern Europe",
    isPopular: false,
    bio: "Elijah is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Cultural Etiquette, Elijah provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Asia Pacific"],
    tourTypes: [
      "Self-Guided Tour",
      "Farm Visit",
      "Off-Road Tour",
      "Market Tour",
      "Cultural Tour",
    ],
    maxGroupSize: 18,
    licenseNumber: "LIC-4629-LON",
    phoneNumber: "+1-909-830-9421",
    contactEmail: "elijah.lewis@paragontrails.com",
    available: [
      {
        day: "Sunday",
        availableHours: [
          {
            from: "11:00",
            to: "17:00",
          },
          {
            from: "15:00",
            to: "21:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "08:00",
            to: "11:00",
          },
        ],
      },
      {
        day: "Monday",
        availableHours: [
          {
            from: "14:00",
            to: "17:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "08:00",
            to: "13:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Elijah is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Cultural Etiquette, Elijah provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.3,
    reviewsCount: 14,
    quote:
      "london isn't just a destination, it's an experience that stays with you forever.",
    profileImage: "https://randomuser.me/api/portraits/women/48.jpg",
    languages: ["Norwegian", "Swedish"],
    experienceYears: 10,
    certifications: ["Eco-Tourism Guide", "CPR & First Aid"],
    specialties: ["Cultural Etiquette"],
    specialTraining: ["Regional Cuisine", "Photography"],
  },
  {
    id: "guide-london-3",
    name: "Ella King",
    city: "london",
    country: "United Kingdom",
    state: "",
    region: "Northern Europe",
    isPopular: true,
    bio: "Ella is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Art Galleries, Ella provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast USA"],
    tourTypes: [
      "Craft Workshop",
      "Cruise Excursion",
      "Romantic Tour",
      "Eco Tour",
      "Cultural Tour",
    ],
    maxGroupSize: 5,
    licenseNumber: "LIC-1552-LON",
    phoneNumber: "+1-662-242-1872",
    contactEmail: "ella.king@paragontrails.com",
    available: [
      {
        day: "Monday",
        availableHours: [
          {
            from: "14:00",
            to: "16:00",
          },
        ],
      },
      {
        day: "Wednesday",
        availableHours: [
          {
            from: "09:00",
            to: "13:00",
          },
        ],
      },
      {
        day: "Thursday",
        availableHours: [
          {
            from: "15:00",
            to: "20:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Ella is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Art Galleries, Ella provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.1,
    reviewsCount: 41,
    quote: "My goal is to help you fall in love with london just as I have.",
    profileImage: "https://randomuser.me/api/portraits/men/79.jpg",
    languages: ["German", "Turkish"],
    experienceYears: 7,
    certifications: [
      "Sustainable Tourism Certificate",
      "Cultural Heritage Specialist",
    ],
    specialties: ["Art Galleries"],
    specialTraining: ["Mountain Survival"],
  },
  {
    id: "guide-london-4",
    name: "Mia Rodriguez",
    city: "london",
    country: "United Kingdom",
    state: "",
    region: "Northern Europe",
    isPopular: false,
    bio: "Mia is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Family Activities, Mia provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast Canada", "West Coast USA"],
    tourTypes: [
      "Boat Tour",
      "Religious Tour",
      "Cultural Tour",
      "Off-Road Tour",
    ],
    maxGroupSize: 13,
    licenseNumber: "LIC-5300-LON",
    phoneNumber: "+1-851-762-9908",
    contactEmail: "mia.rodriguez@paragontrails.com",
    available: [
      {
        day: "Sunday",
        availableHours: [
          {
            from: "08:00",
            to: "12:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "09:00",
            to: "12:00",
          },
          {
            from: "10:00",
            to: "14:00",
          },
        ],
      },
      {
        day: "Saturday",
        availableHours: [
          {
            from: "13:00",
            to: "16:00",
          },
          {
            from: "10:00",
            to: "13:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Mia is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Family Activities, Mia provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.1,
    reviewsCount: 70,
    quote:
      "london isn't just a destination, it's an experience that stays with you forever.",
    profileImage: "https://randomuser.me/api/portraits/men/34.jpg",
    languages: ["Malay"],
    experienceYears: 12,
    certifications: ["Urban Navigation Specialist"],
    specialties: ["Family Activities"],
    specialTraining: ["Conflict Resolution", "Wine Education"],
  },
  {
    id: "guide-london-5",
    name: "Evelyn Young",
    city: "london",
    country: "United Kingdom",
    state: "",
    region: "Northern Europe",
    isPopular: false,
    bio: "Evelyn is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Historical Landmarks, LGBTQ+ History, Adventure Sports, Family Activities, Evelyn provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Asia Pacific"],
    tourTypes: ["Architecture Tour", "Museum Tour"],
    maxGroupSize: 12,
    licenseNumber: "LIC-2187-LON",
    phoneNumber: "+1-437-380-6787",
    contactEmail: "evelyn.young@paragontrails.com",
    available: [
      {
        day: "Saturday",
        availableHours: [
          {
            from: "10:00",
            to: "13:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "08:00",
            to: "11:00",
          },
          {
            from: "15:00",
            to: "20:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "09:00",
            to: "15:00",
          },
          {
            from: "12:00",
            to: "16:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Evelyn is a passionate tour guide with extensive knowledge of london and United Kingdom. Specializing in Historical Landmarks, LGBTQ+ History, Adventure Sports, Family Activities, Evelyn provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.3,
    reviewsCount: 90,
    quote: "My goal is to help you fall in love with london just as I have.",
    profileImage: "https://randomuser.me/api/portraits/women/0.jpg",
    languages: ["Tagalog", "Croatian", "Norwegian"],
    experienceYears: 12,
    certifications: [
      "Professional Tour Guide Association",
      "Eco-Tourism Guide",
      "Wilderness First Responder",
    ],
    specialties: [
      "Historical Landmarks",
      "LGBTQ+ History",
      "Adventure Sports",
      "Family Activities",
    ],
    specialTraining: ["Local Flora & Fauna", "Photography"],
  },
];
