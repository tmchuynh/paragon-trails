import { TourGuide } from "@/lib/interfaces/people/staff";

export const glacierBayunitedStatesalaskaGuides: TourGuide[] = [
  {
    id: "guide-glacier-bay-1",
    name: "Henry Miller",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: true,
    bio: "Henry is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Wildlife, Parks & Gardens, Henry provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast Canada"],
    tourTypes: ["Accessible Tour", "Nature Tour", "Farm Visit", "Eco Tour"],
    maxGroupSize: 13,
    licenseNumber: "LIC-7735-GLA",
    phoneNumber: "+1-210-348-6291",
    contactEmail: "henry.miller@paragontrails.com",
    available: [],
    isCertified: true,
    description:
      "Henry is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Wildlife, Parks & Gardens, Henry provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 3.6,
    reviewsCount: 65,
    quote: "In Glacier Bay, we don't just see the sights - we make memories.",
    profileImage: "https://randomuser.me/api/portraits/women/70.jpg",
    languages: ["Croatian", "Norwegian", "Russian"],
    experienceYears: 13,
    certifications: ["Urban Navigation Specialist", "CPR & First Aid"],
    specialties: ["Wildlife", "Parks & Gardens"],
    specialTraining: ["Accessibility Services"],
  },
  {
    id: "guide-glacier-bay-2",
    name: "Samuel Anderson",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: false,
    bio: "Samuel is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Religious Sites, Wildlife, Historical Landmarks, Samuel provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["South America"],
    tourTypes: [
      "Off-Road Tour",
      "Ghost Tour",
      "Scenic Tour",
      "Cruise Excursion",
    ],
    maxGroupSize: 18,
    licenseNumber: "LIC-3791-GLA",
    phoneNumber: "+1-511-285-6384",
    contactEmail: "samuel.anderson@paragontrails.com",
    available: [],
    isCertified: true,
    description:
      "Samuel is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Religious Sites, Wildlife, Historical Landmarks, Samuel provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.8,
    reviewsCount: 31,
    quote:
      "Glacier Bay isn't just a destination, it's an experience that stays with you forever.",
    profileImage: "https://randomuser.me/api/portraits/men/27.jpg",
    languages: ["Hungarian", "Czech"],
    experienceYears: 8,
    certifications: ["Certified International Tour Manager"],
    specialties: ["Religious Sites", "Wildlife", "Historical Landmarks"],
    specialTraining: ["Accessibility Services", "Regional Cuisine"],
  },
  {
    id: "guide-glacier-bay-3",
    name: "Joseph Johnson",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: false,
    bio: "Joseph is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Public Transportation Navigation, Film Locations, Joseph provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Mediterranean"],
    tourTypes: [
      "Adventure Tour",
      "Scenic Tour",
      "Brewery Tour",
      "Festival Tour",
    ],
    maxGroupSize: 12,
    licenseNumber: "LIC-2142-GLA",
    phoneNumber: "+1-749-748-8492",
    contactEmail: "joseph.johnson@paragontrails.com",
    available: [],
    isCertified: true,
    description:
      "Joseph is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Public Transportation Navigation, Film Locations, Joseph provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4,
    reviewsCount: 96,
    quote:
      "My goal is to help you fall in love with Glacier Bay just as I have.",
    profileImage: "https://randomuser.me/api/portraits/women/5.jpg",
    languages: ["Greek", "Thai"],
    experienceYears: 4,
    certifications: ["Eco-Tourism Guide"],
    specialties: ["Public Transportation Navigation", "Film Locations"],
    specialTraining: [],
  },
  {
    id: "guide-glacier-bay-4",
    name: "Amelia Wright",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: true,
    bio: "Amelia is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Nightlife, Street Food, Amelia provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast USA"],
    tourTypes: [
      "Helicopter Tour",
      "Architecture Tour",
      "Boat Tour",
      "Museum Tour",
      "Shopping Tour",
    ],
    maxGroupSize: 15,
    licenseNumber: "LIC-3236-GLA",
    phoneNumber: "+1-276-491-7823",
    contactEmail: "amelia.wright@paragontrails.com",
    available: [],
    isCertified: true,
    description:
      "Amelia is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Nightlife, Street Food, Amelia provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.1,
    reviewsCount: 43,
    quote:
      "Glacier Bay isn't just a destination, it's an experience that stays with you forever.",
    profileImage: "https://randomuser.me/api/portraits/men/45.jpg",
    languages: ["Vietnamese"],
    experienceYears: 7,
    certifications: ["Professional Tour Guide Association", "CPR & First Aid"],
    specialties: ["Nightlife", "Street Food"],
    specialTraining: ["Art History", "Emergency Response"],
  },
  {
    id: "guide-glacier-bay-5",
    name: "David Green",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: true,
    bio: "David is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Cultural Etiquette, David provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Mediterranean", "South America"],
    tourTypes: ["Romantic Tour", "Photography Tour", "Bike Tour"],
    maxGroupSize: 6,
    licenseNumber: "LIC-9773-GLA",
    phoneNumber: "+1-709-281-6270",
    contactEmail: "david.green@paragontrails.com",
    available: [
      {
        day: "Saturday",
        availableHours: [
          {
            from: "08:00",
            to: "12:00",
          },
        ],
      },
      {
        day: "Thursday",
        availableHours: [
          {
            from: "11:00",
            to: "13:00",
          },
          {
            from: "14:00",
            to: "17:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "11:00",
            to: "16:00",
          },
          {
            from: "15:00",
            to: "19:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "David is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Cultural Etiquette, David provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.2,
    reviewsCount: 81,
    quote:
      "Every corner of Glacier Bay tells a story, let me share them with you.",
    profileImage: "https://randomuser.me/api/portraits/women/36.jpg",
    languages: ["Arabic", "Mandarin"],
    experienceYears: 2,
    certifications: [
      "Local History Certification",
      "Certified International Tour Manager",
      "Urban Navigation Specialist",
    ],
    specialties: ["Cultural Etiquette"],
    specialTraining: ["Historical Research Methods"],
  },
  {
    id: "guide-glacier-bay-6",
    name: "Sofia Jones",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: true,
    bio: "Sofia is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Hidden Gems, Urban Legends, Art Galleries, Sofia provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast USA"],
    tourTypes: ["Audio Tour", "Luxury Tour"],
    maxGroupSize: 14,
    licenseNumber: "LIC-3755-GLA",
    phoneNumber: "+1-654-755-7952",
    contactEmail: "sofia.jones@paragontrails.com",
    available: [
      {
        day: "Sunday",
        availableHours: [
          {
            from: "14:00",
            to: "19:00",
          },
          {
            from: "08:00",
            to: "11:00",
          },
        ],
      },
      {
        day: "Monday",
        availableHours: [
          {
            from: "09:00",
            to: "13:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "11:00",
            to: "15:00",
          },
          {
            from: "15:00",
            to: "19:00",
          },
        ],
      },
      {
        day: "Thursday",
        availableHours: [
          {
            from: "13:00",
            to: "18:00",
          },
          {
            from: "12:00",
            to: "16:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Sofia is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in Hidden Gems, Urban Legends, Art Galleries, Sofia provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 3.9,
    reviewsCount: 33,
    quote:
      "My goal is to help you fall in love with Glacier Bay just as I have.",
    profileImage: "https://randomuser.me/api/portraits/women/77.jpg",
    languages: ["Norwegian", "Danish", "Swedish"],
    experienceYears: 14,
    certifications: [
      "Regional Tourism Ambassador",
      "Heritage Interpretation Certificate",
      "Food Safety Certification",
    ],
    specialties: ["Hidden Gems", "Urban Legends", "Art Galleries"],
    specialTraining: ["Accessibility Services", "Marine Biology"],
  },
  {
    id: "guide-glacier-bay-7",
    name: "Evelyn Davis",
    city: "Glacier Bay",
    country: "United States",
    state: "",
    region: "Alaska",
    isPopular: false,
    bio: "Evelyn is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in LGBTQ+ History, Hidden Gems, Evelyn provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast Canada"],
    tourTypes: [
      "Festival Tour",
      "Cultural Tour",
      "Ghost Tour",
      "Cruise Excursion",
      "Private Tour",
    ],
    maxGroupSize: 13,
    licenseNumber: "LIC-7108-GLA",
    phoneNumber: "+1-111-311-4478",
    contactEmail: "evelyn.davis@paragontrails.com",
    available: [
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "08:00",
            to: "12:00",
          },
        ],
      },
      {
        day: "Monday",
        availableHours: [
          {
            from: "13:00",
            to: "18:00",
          },
          {
            from: "12:00",
            to: "15:00",
          },
        ],
      },
      {
        day: "Thursday",
        availableHours: [
          {
            from: "08:00",
            to: "14:00",
          },
          {
            from: "13:00",
            to: "17:00",
          },
        ],
      },
      {
        day: "Saturday",
        availableHours: [
          {
            from: "09:00",
            to: "14:00",
          },
          {
            from: "08:00",
            to: "10:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Evelyn is a passionate tour guide with extensive knowledge of Glacier Bay and United States. Specializing in LGBTQ+ History, Hidden Gems, Evelyn provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.7,
    reviewsCount: 67,
    quote:
      "Glacier Bay isn't just a destination, it's an experience that stays with you forever.",
    profileImage: "https://randomuser.me/api/portraits/women/5.jpg",
    languages: ["Mandarin", "Finnish"],
    experienceYears: 8,
    certifications: ["Certified International Tour Manager"],
    specialties: ["LGBTQ+ History", "Hidden Gems"],
    specialTraining: ["Archaeological Conservation", "Local Flora & Fauna"],
  },
];
