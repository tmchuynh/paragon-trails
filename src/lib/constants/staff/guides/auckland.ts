import { TourGuide } from "@/lib/interfaces/people/staff";

export const aucklandnewZealandasiaPacificGuides: TourGuide[] = [
  {
    id: "guide-auckland-1",
    name: "Emma Jackson",
    city: "auckland",
    country: "New Zealand",
    state: "",
    region: "Asia Pacific",
    isPopular: true,
    bio: "Emma is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Indigenous Culture, Photography Spots, Public Transportation Navigation, Emma provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Asia Pacific", "Mediterranean"],
    tourTypes: [
      "Ghost Tour",
      "Food Tour",
      "LGBTQ+ Friendly Tour",
      "Craft Workshop",
    ],
    maxGroupSize: 5,
    licenseNumber: "LIC-5410-AUC",
    phoneNumber: "+1-317-365-8711",
    contactEmail: "emma.jackson@paragontrails.com",
    available: [
      {
        day: "Thursday",
        availableHours: [
          {
            from: "10:00",
            to: "16:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "15:00",
            to: "17:00",
          },
        ],
      },
      {
        day: "Saturday",
        availableHours: [
          {
            from: "08:00",
            to: "14:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Emma is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Indigenous Culture, Photography Spots, Public Transportation Navigation, Emma provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4.6,
    reviewsCount: 34,
    quote:
      "The best way to discover auckland is with someone who lives and breathes its culture.",
    profileImage: "https://randomuser.me/api/portraits/women/62.jpg",
    languages: ["Malay", "Tagalog"],
    experienceYears: 7,
    certifications: [
      "CPR & First Aid",
      "Eco-Tourism Guide",
      "Professional Tour Guide Association",
    ],
    specialties: [
      "Indigenous Culture",
      "Photography Spots",
      "Public Transportation Navigation",
    ],
    specialTraining: ["Regional Cuisine", "Photography"],
  },
  {
    id: "guide-auckland-2",
    name: "Emily Thomas",
    city: "auckland",
    country: "New Zealand",
    state: "",
    region: "Asia Pacific",
    isPopular: false,
    bio: "Emily is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Urban Legends, Nightlife, Local Craftsmanship, Emily provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["West Coast USA", "East Coast USA"],
    tourTypes: ["Private Tour", "Wine Tour", "Relocation Tour"],
    maxGroupSize: 11,
    licenseNumber: "LIC-3157-AUC",
    phoneNumber: "+1-410-576-3072",
    contactEmail: "emily.thomas@paragontrails.com",
    available: [
      {
        day: "Sunday",
        availableHours: [
          {
            from: "11:00",
            to: "13:00",
          },
        ],
      },
      {
        day: "Monday",
        availableHours: [
          {
            from: "12:00",
            to: "17:00",
          },
          {
            from: "13:00",
            to: "15:00",
          },
        ],
      },
      {
        day: "Wednesday",
        availableHours: [
          {
            from: "12:00",
            to: "16:00",
          },
          {
            from: "11:00",
            to: "14:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "13:00",
            to: "19:00",
          },
          {
            from: "08:00",
            to: "11:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Emily is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Urban Legends, Nightlife, Local Craftsmanship, Emily provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 3.9,
    reviewsCount: 50,
    quote: "In auckland, we don't just see the sights - we make memories.",
    profileImage: "https://randomuser.me/api/portraits/women/78.jpg",
    languages: ["Hungarian"],
    experienceYears: 12,
    certifications: ["CPR & First Aid", "Cultural Heritage Specialist"],
    specialties: ["Urban Legends", "Nightlife", "Local Craftsmanship"],
    specialTraining: ["Religious Studies", "Local Transit Systems"],
  },
  {
    id: "guide-auckland-3",
    name: "Isabella Clark",
    city: "auckland",
    country: "New Zealand",
    state: "",
    region: "Asia Pacific",
    isPopular: false,
    bio: "Isabella is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Photography Spots, Cultural Etiquette, Sustainable Tourism, Isabella provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["East Coast USA", "Mediterranean"],
    tourTypes: ["Market Tour", "Ghost Tour", "Boat Tour", "Street Art Tour"],
    maxGroupSize: 9,
    licenseNumber: "LIC-9917-AUC",
    phoneNumber: "+1-235-551-1482",
    contactEmail: "isabella.clark@paragontrails.com",
    available: [
      {
        day: "Monday",
        availableHours: [
          {
            from: "11:00",
            to: "15:00",
          },
          {
            from: "10:00",
            to: "14:00",
          },
        ],
      },
      {
        day: "Sunday",
        availableHours: [
          {
            from: "13:00",
            to: "17:00",
          },
          {
            from: "13:00",
            to: "18:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "11:00",
            to: "15:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "15:00",
            to: "19:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Isabella is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Photography Spots, Cultural Etiquette, Sustainable Tourism, Isabella provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4,
    reviewsCount: 75,
    quote:
      "Every corner of auckland tells a story, let me share them with you.",
    profileImage: "https://randomuser.me/api/portraits/men/22.jpg",
    languages: ["Czech", "Tagalog"],
    experienceYears: 15,
    certifications: [
      "Certified International Tour Manager",
      "Adventure Tourism Safety Certificate",
      "Language Proficiency Certification",
    ],
    specialties: [
      "Photography Spots",
      "Cultural Etiquette",
      "Sustainable Tourism",
    ],
    specialTraining: ["Emergency Response"],
  },
  {
    id: "guide-auckland-4",
    name: "Amelia Taylor",
    city: "auckland",
    country: "New Zealand",
    state: "",
    region: "Asia Pacific",
    isPopular: false,
    bio: "Amelia is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Music Scene, Art Galleries, LGBTQ+ History, Nightlife, Amelia provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["Northern Europe"],
    tourTypes: [
      "Cultural Tour",
      "Romantic Tour",
      "Photography Tour",
      "Festival Tour",
    ],
    maxGroupSize: 17,
    licenseNumber: "LIC-3197-AUC",
    phoneNumber: "+1-797-113-7276",
    contactEmail: "amelia.taylor@paragontrails.com",
    available: [
      {
        day: "Saturday",
        availableHours: [
          {
            from: "09:00",
            to: "15:00",
          },
          {
            from: "14:00",
            to: "17:00",
          },
        ],
      },
      {
        day: "Thursday",
        availableHours: [
          {
            from: "15:00",
            to: "19:00",
          },
          {
            from: "15:00",
            to: "17:00",
          },
        ],
      },
      {
        day: "Wednesday",
        availableHours: [
          {
            from: "11:00",
            to: "13:00",
          },
        ],
      },
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "11:00",
            to: "16:00",
          },
          {
            from: "15:00",
            to: "19:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Amelia is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Music Scene, Art Galleries, LGBTQ+ History, Nightlife, Amelia provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 3.7,
    reviewsCount: 34,
    quote: "My goal is to help you fall in love with auckland just as I have.",
    profileImage: "https://randomuser.me/api/portraits/women/22.jpg",
    languages: ["Tagalog"],
    experienceYears: 2,
    certifications: [
      "Certified International Tour Manager",
      "Licensed City Guide",
      "Wilderness First Responder",
    ],
    specialties: [
      "Music Scene",
      "Art Galleries",
      "LGBTQ+ History",
      "Nightlife",
    ],
    specialTraining: ["Accessibility Services"],
  },
  {
    id: "guide-auckland-5",
    name: "Elijah Carter",
    city: "auckland",
    country: "New Zealand",
    state: "",
    region: "Asia Pacific",
    isPopular: false,
    bio: "Elijah is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Religious Sites, Sustainable Tourism, Adventure Sports, Family Activities, Elijah provides authentic and engaging experiences for visitors of all backgrounds.",
    regionsCovered: ["West Coast USA", "East Coast USA"],
    tourTypes: [
      "Family-Friendly Tour",
      "Audio Tour",
      "Religious Tour",
      "Farm Visit",
      "Adventure Tour",
    ],
    maxGroupSize: 7,
    licenseNumber: "LIC-6720-AUC",
    phoneNumber: "+1-938-282-2835",
    contactEmail: "elijah.carter@paragontrails.com",
    available: [
      {
        day: "Tuesday",
        availableHours: [
          {
            from: "15:00",
            to: "17:00",
          },
          {
            from: "12:00",
            to: "18:00",
          },
        ],
      },
      {
        day: "Wednesday",
        availableHours: [
          {
            from: "11:00",
            to: "16:00",
          },
        ],
      },
      {
        day: "Friday",
        availableHours: [
          {
            from: "08:00",
            to: "14:00",
          },
        ],
      },
      {
        day: "Thursday",
        availableHours: [
          {
            from: "09:00",
            to: "13:00",
          },
          {
            from: "13:00",
            to: "15:00",
          },
        ],
      },
    ],
    isCertified: true,
    description:
      "Elijah is a passionate tour guide with extensive knowledge of auckland and New Zealand. Specializing in Religious Sites, Sustainable Tourism, Adventure Sports, Family Activities, Elijah provides authentic and engaging experiences for visitors of all backgrounds.",
    rating: 4,
    reviewsCount: 36,
    quote:
      "auckland isn't just a destination, it's an experience that stays with you forever.",
    profileImage: "https://randomuser.me/api/portraits/men/0.jpg",
    languages: ["Portuguese", "Arabic", "Italian"],
    experienceYears: 7,
    certifications: ["Food Safety Certification"],
    specialties: [
      "Religious Sites",
      "Sustainable Tourism",
      "Adventure Sports",
      "Family Activities",
    ],
    specialTraining: ["Urban Planning"],
  },
];
