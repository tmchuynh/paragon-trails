...
export async function fetchReviews(postId?: number, limit?: number): Promise<any[]> {
  ...
  try {
    ...
    const reviews = comments.map(comment => ({
      ...
      reviewDate: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000).toISOString(),
      reviewer: {
        ...
        memberSince: new Date(Date.now() - Math.random() * 1000 * 24 * 60 * 60 * 1000).toISOString(),
      },
    }));
...
export async function fetchDestinationGalleries(limit?: number): Promise<any[]> {
...
function convertToTravelTitle(title: string): string {
  ...
  const prefix = travelPrefixes[Math.floor(Math.random() * travelPrefixes.length)];
...
function getTravelCategory(): string {
  const categories = ['Adventure', 'Culture', 'Food', 'Nature', 'City', 'Beach', 'Mountain', 'Historical'];
  return categories[Math.floor(Math.random() * categories.length)];
}

function getTravelTags(): string[] {
  const allTags = ['travel', 'adventure', 'culture', 'food', 'nature', 'photography', 'tips', 'guide', 'destination', 'vacation'];
  const count = Math.floor(Math.random() * 4) + 2;
  return allTags.slice(0, count);
}

function getReviewableType(): string {
  const types = ['hotel', 'restaurant', 'attraction', 'tour', 'activity', 'destination'];
...
function getItineraryCategory(): string {
  const categories = ['Sightseeing', 'Dining', 'Transportation', 'Accommodation', 'Activities', 'Preparation'];
  return categories[Math.floor(Math.random() * categories.length)];
}
