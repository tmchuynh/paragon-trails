...
export default function ShoppingPage({ params }: ShoppingPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadCityShopping() {
      try {
        const result = await fetchAttractionsByCity({
          ...
          categories:
            "commercial,pet,healthcare.pharmacy,service.beauty,service.tailor,service.locksmith",
          limit: 60,
        });
        setData(result);
        setFilteredAttractions(result.attractions);
      } catch (error) {
        console.error("Error loading city shopping:", error);
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="mx-auto px-4 py-8 container">
        <div className="py-16 text-center">
          <ShoppingBag className="mx-auto mb-4 w-16 h-16 text-muted-foreground" />
  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto px-6 lg:px-8 py-12 max-w-7xl">
        ...
        {/* City Header */}
        <div className="mb-8">
          <h1 className="mb-2 font-bold text-4xl">Shopping in {city.name}</h1>
          <div className="flex items-center gap-2 text-muted-foreground">
            <ShoppingBag className="w-4 h-4" />
            <span>Stores, Services & Commercial Places</span>
            <span>•</span>
        ...
        {filteredAttractions.length > 0 ? (
          <>
            <div className="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {paginatedAttractions.map((attraction) => (
                <ShopCard key={attraction.id} attraction={attraction} />
              ))}
            </div>
          ...
          <div className="py-12 text-center">
            <ShoppingBag className="mx-auto mb-4 w-12 h-12 text-muted-foreground" />
            <h3 className="mb-2 font-medium text-lg">No Shops Found</h3>
            <p className="text-muted-foreground">
              {hasActiveFilters
                ? "No shops or services match your current filters."
                : `No shops or services available for ${city.name}.`}
            </p>
            {hasActiveFilters && (
...
function ShopCard({ attraction }: { attraction: Attraction }) {
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
          ...
          <Badge
            ...
            className="text-xs"
          >
            {toTitleCase(attraction.categories[0]?.split(".")[0]) || "Shop"}
          </Badge>
        </div>
      </CardHeader>
