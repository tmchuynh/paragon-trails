...
export default function RentalsPage({ params }: RentalsPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadCityRentals() {
      try {
        ...
        console.error("Error loading city rentals:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto px-6 lg:px-8 py-12 max-w-7xl">
        ...
        {/* City Header */}
        <div className="mb-8">
          <h1 className="mb-2 font-bold text-4xl">Rentals in {city.name}</h1>
          <div className="flex items-center gap-2 text-muted-foreground">
            <Car className="w-4 h-4" />
            <span>Car Rentals, Bike Rentals & Equipment</span>
            <span>•</span>
        ...
        {filteredAttractions.length > 0 ? (
          <>
            <div className="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {paginatedAttractions.map((attraction) => (
                <RentalCard key={attraction.id} attraction={attraction} />
              ))}
            </div>
          ...
          <div className="py-12 text-center">
            <Car className="mx-auto mb-4 w-12 h-12 text-muted-foreground" />
            <h3 className="mb-2 font-medium text-lg">
              No Rental Services Found
            </h3>
            <p className="text-muted-foreground">
              {hasActiveFilters
                ? "No rental services match your current filters."
                : `No rental services available for ${city.name}.`}
            </p>
            {hasActiveFilters && (
...
function RentalCard({ attraction }: { attraction: Attraction }) {
