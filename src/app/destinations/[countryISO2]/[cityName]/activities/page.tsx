    ...
    async function loadCityActivities() {
      try {
        const result = await fetchAttractionsByCity({
          ...
          categories:
            "entertainment,leisure,adult,ski,sport,religion,activity,natural,national_park,commercial.shopping_mall,commercial.outdoor_and_sport,commercial.hobby,commercial.books,commercial.gift_and_souvenir,commercial.weapons",
          limit: 60,
        });
        setData(result);
        setFilteredAttractions(result.attractions);
      } catch (error) {
        console.error("Error loading city activities:", error);
      } finally {
        setLoading(false);
      }
    }
    ...
    return (
      <div className="mx-auto px-4 py-8 container">
        <div className="py-16 text-center">
          <Activity className="mx-auto mb-4 w-16 h-16 text-muted-foreground" />
  ...
  return (
    <div className="min-h-screen">
      <div className="mx-auto px-6 lg:px-8 py-12 max-w-7xl">
        ...
        {/* City Header */}
        <div className="mb-8">
          <h1 className="mb-2 font-bold text-4xl">Activities in {city.name}</h1>
          <div className="flex items-center gap-2 text-muted-foreground">
            <Activity className="w-4 h-4" />
            <span>Entertainment, Sports, Leisure & Activities</span>
            <span>•</span>
        ...
        {filteredAttractions.length > 0 ? (
          ...
          <div className="py-12 text-center">
            <Activity className="mx-auto mb-4 w-12 h-12 text-muted-foreground" />
            <h3 className="mb-2 font-medium text-lg">No Activities Found</h3>
            <p className="text-muted-foreground">
              {hasActiveFilters
                ? "No activities match your current filters."
                : `No activities available for ${city.name}.`}
            </p>
            {hasActiveFilters && (
...
function ActivityCard({ attraction }: { attraction: Attraction }) {
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
          ...
          <Badge
            ...
            className="text-xs"
          >
            {toTitleCase(attraction.categories[0]?.split(".")[0]) || "Activity"}
          </Badge>
        </div>
      </CardHeader>
