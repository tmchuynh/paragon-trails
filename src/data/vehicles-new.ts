...
export const getMockVehicles = async () => {
  ...
  try {
    ...
    const cars = nHTSAService.transformMakesToVehicles(
      carMakes.slice(0, 15),
      "car"
    );
    const trucks = nHTSAService.transformMakesToVehicles(
      truckMakes.slice(0, 10),
      "truck"
    );
    const motorcycles = nHTSAService.transformMakesToVehicles(
      motorcycleMakes.slice(0, 10),
      "motorcycle"
    );
    const suvs = nHTSAService.transformMakesToVehicles(
      suvMakes.slice(0, 15),
      "suv"
    );
...
const getFallbackVehicles = () => [
  ...
  },
  {
    ...
    description:
      "America's best-selling truck, perfect for work and adventure.",
    specifications: {
      engine: "3.5L V6",
      transmission: "Automatic",
      fuelType: "Gasoline",
